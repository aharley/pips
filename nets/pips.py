...
def score_map_loss(fcps, trajs_g, vis_g, valids):
    ...
    ind = (
        (x_ >= 0) & (x_ <= (W8 - 1)) & (y_ >= 0) & (y_ <= (H8 - 1)) & (valid_ > 0) & (vis_ > 0)
    )  # BSN
...
class ResidualBlock(nn.Module):
    def __init__(self, in_planes, planes, norm_fn="group", stride=1):
        ...
        self.conv1 = nn.Conv2d(
            in_planes, planes, kernel_size=3, padding=1, stride=stride, padding_mode="zeros"
        )
...
class BasicEncoder(nn.Module):
    def __init__(self, input_dim=3, output_dim=128, stride=8, norm_fn="batch", dropout=0.0):
        ...
        self.conv1 = nn.Conv2d(
            input_dim, self.in_planes, kernel_size=7, stride=2, padding=3, padding_mode="zeros"
        )
        ...
        if self.shallow:
            ...
            self.conv2 = nn.Conv2d(
                128 + 128 + 96 + 64, output_dim * 2, kernel_size=3, padding=1, padding_mode="zeros"
            )
...
class Pips(nn.Module):
    ...
    def forward(
        ...
        if feat_init is None:
            ...
            ffeat = utils.samp.bilinear_sample2d(
        ...
        if sw is not None and sw.save_this:
            ...
            for s in range(S):
                if trajs_g is not None:
                    ...
                    kp = utils.improc.draw_circles_at_xy(
        ...
        for itr in range(iters):
            ...
            times_ = (
                torch.linspace(0, S, S, device=device).reshape(1, S, 1).repeat(B * N, 1, 1)
            )  # B*N,S,1
